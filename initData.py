...
def initPlottedData(parent, progressBar, currentMask, thread):
    ...
    allMarkers = np.linspace(0, parent.nb_marker, parent.nb_marker, endpoint=False)
    activeMarkers = []
    activeImages = []
    ...
    for image in range(0, nbImages):
        ...
        for instance in range(nbInstances):
            ...
            for marker in instanceMarkers:
                activeNeighbors = [neighbor for neighbor in neighbors[marker] if neighbor in instanceMarkers]
